...
import {
  AboutBox, CardDevBtn, DatatableHome, DevHomeHeadTile,
...
import { setDeviceId, setSerial, setSearchQuery, setShowAlert } from "../../stores/utilsStateSlice"
...
import { DevicesType,  HomeDeviceType, ProbeType } from "../../types/smtrack/devices.type"
...
export default function Home() {
  ...
  const { searchQuery, hosId, wardId, userProfile, tokenDecode, transparent, socketData } = useSelector((state: RootState) => state.utilsState)
  ...
  const fetchDevices = useCallback(async (page: number, size = perPage) => {
    try {
      ...
      const response = await axiosInstance.get<responseType<DevicesType>>(`/devices/device?${wardId ? `ward=${wardId}&` : ''}page=${page}&perpage=${size}`)
  ...
  const columns: TableColumn<HomeDeviceType>[] = [
    ...
    },
    {
      ...
      cell: items => <span title={items.location ?? '- -'}>{items.location ?? '- -'}</span>,
      sortable: false,
      center: true,
      width: '200px',
    },
    {
      ...
      cell: items => (
        <span key={items.id}>
          {items.log.length > 0 ? `${items.log[0].tempDisplay.toFixed(2)}°C` : '- -'}
        </span>
      ),
      sortable: false,
      center: true,
    ...
    },
    {
      name: t('deviceHumiTb'),
      selector: items =>
        items.log.length > 0 ? `${items.log[0].humidityDisplay.toFixed(2)}%` : '- -',
      sortable: false,
      center: true,
      width: '85px',
    },
    {
      ...
      cell: items => {
        ...
        const isTempOutOfRange = temp?.tempDisplay >= probe?.tempMax || temp?.tempDisplay <= probe?.tempMin
        ...
        return (
          <DeviceCardFooterDoorFlex key={items.id} $primary>
            {doors.slice(0, doorCount).map(doorKey => (
              <DeviceCardFooterDoor $primary={items.log[0]?.[doorKey]} key={doorKey}>
                {items.log[0]?.[doorKey] ? <RiDoorOpenLine /> : <RiDoorClosedLine />}
  ...
  const subDeviceColumns: TableColumn<ProbeType>[] = [
    ...
    },
    {
      ...
      cell: (items, index) => {
        ...
        return (
          <span key={index}>
            {deviceLog?.tempDisplay ? `${deviceLog?.tempDisplay.toFixed(2)}°C` : '- -'}
          </span>
        );
      },
      sortable: false,
    ...
    },
    {
      ...
      cell: (items, index) => {
        ...
        return (
          <span key={index}>
            {deviceLog?.humidityDisplay ? `${deviceLog?.humidityDisplay.toFixed(2)}%` : '- -'}
          </span>
        );
      },
      sortable: false,
    ...
    },
    {
      ...
      cell: items => {
        ...
        const isTempOutOfRange =
          deviceLog!?.tempDisplay >= items.tempMax || deviceLog!?.tempDisplay <= items.tempMin
      ...
      cell: items => {
        ...
        return (
          <DeviceCardFooterDoorFlex $primary>
            {Array.from({ length: items.doorQty }, (_, i) => renderDoor(`door${i + 1}` as 'door1'))}
          </DeviceCardFooterDoorFlex>
        );
      },
      sortable: false,
