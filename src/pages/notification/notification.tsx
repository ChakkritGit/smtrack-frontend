  ...
  const fetchData = async () => {
    const baseUrl =
      role === 'LEGACY_ADMIN' || role === 'LEGACY_USER' || isTms
        ? `${import.meta.env.VITE_APP_API}/legacy/templog/alert/notification`
        : `${import.meta.env.VITE_APP_API}/notification`
    try {
      ...
      toast(
        _t => (
          <ToastContainer>
            ...
            <button onClick={() => toast.dismiss(_t.id)}>
  ...
  return (
    <>
      ...
      <Modal scrollable size='lg' show={show} onHide={closeModal}>
        <Modal.Header>
          <ModalHead>
            <strong>{t('titleNotification')}</strong>
        ...
        <Modal.Body style={{ padding: 'unset' }}>
          <Notificationdata data={notiData} funcfetch={fetchData} />
        </Modal.Body>
      </Modal>
    </>
  )
