...
export default function Warranty () {
  ...
  const { searchQuery, cookieDecode, tokenDecode } = useSelector(
    (state: RootState) => state.utilsState
  )
  const hospitalsData = useSelector<DeviceStateStore, hospitalsType[]>(
    state => state.arraySlice.hospital.hospitalsData
  )
  ...
  const { devices } = useSelector<DeviceStateStore, DeviceState>(
    state => state.devices
  )
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true)
      const response = await axiosInstance.get<responseType<warrantyType[]>>(
        `${import.meta.env.VITE_APP_API}/warranty`
      )
  ...
  const devicesArray = warrantyData.filter(
    items =>
      items.device.devSerial
        .toLowerCase()
        .includes(searchQuery.toLowerCase()) ||
      items.device.devDetail?.toLowerCase().includes(searchQuery.toLowerCase())
  )
  ...
  const deleteWarranty = async (wId: string) => {
    try {
      const response = await axiosInstance.delete(
        `${import.meta.env.VITE_APP_API}/warranty/${wId}`
      )
      Swal.fire({
  ...
  const columns: TableColumn<warrantyType>[] = [
    ...
    },
    {
      name: t('deviceDate'),
      selector: items =>
        `${new Date(String(items.installDate)).toLocaleString('th-TH', {
          day: '2-digit',
          month: '2-digit',
          year: 'numeric',
          timeZone: 'UTC'
                ...
                <AddWarrantyButton
                  onClick={() => {
                    ...
                    setWarrantyObject({
                      ...warrantyObject,
                      wardId: items.warrId,
                      devName: String(items.devName),
                      expireData: items.expire.substring(0, 10),
                      invoice: String(items.invoice),
                      customerAddress: String(items.customerAddress),
                      customerName: String(items.customerName),
                      devSerial: items.device.devSerial,
                      installDate: String(items.installDate),
                      productName: String(items.productName),
                      saleDept: String(items.saleDept)
                    })
                  }}
                  $primary
                >
  ...
  const setHosId = (e: SingleValue<Option>) => {
    ...
    setWarrantyObject({
      ...
      customerAddress: String(
        hospitalsData
          .filter(items =>
            items.id.toLowerCase().includes(selectedValue.toLowerCase())
          )
          .map(items => items.hosAddress)[0]
      ),
      customerName: String(selectedLabel)
    })
  }
    ...
    if (
      ...
      try {
        const response = await axiosInstance.put(
          `${import.meta.env.VITE_APP_API}/warranty/${wardId}`,
          body
        )
        Swal.fire({
          title: t('alertHeaderSuccess'),
  ...
  const mapOptions = <T extends WithCount, K extends keyof T>(
    data: T[],
    valueKey: K,
    labelKey: K
  ): Option[] =>
    data
      .filter(item => item._count?.warranty === 0)
      .map(item => ({
  ...
  const mapOptionsHospital = <T, K extends keyof T>(
    data: T[],
    valueKey: K,
    labelKey: K
  ): Option[] =>
  ...
  const mapDefaultValueHospital = <T, K extends keyof T>(
    data: T[],
    id: string,
    valueKey: K,
    labelKey: K
    ...
    <Container fluid>
      <WarrantyHead>
        <div>
          ...
          <WarrantyHeadBtn
            $primary={pagenumber === 2}
            onClick={() => setpagenumber(2)}
          >
            {t('tabWarrantyAfterSale')}
          </WarrantyHeadBtn>
          <WarrantyHeadBtn
            $primary={pagenumber === 3}
      ...
      <Modal size='lg' show={showTwo} onHide={closeModalTwo}>
        <Modal.Header>
          <ModalHead>
            <strong>
              {pagestate === 'add' ? t('addWarrantyButton') : t('editWarranty')}
            </strong>
        ...
        <Form onSubmit={pagestate === 'add' ? handleSubmit : handleSubmitEdit}>
            ...
            <Row>
              ...
              {pagestate === 'add' && (
                <Col lg={12}>
                  <Form.Label className='w-100'>
                    {t('selectDeviceDrop')}
                    <Select
                      options={mapOptions<WarrantyOption, keyof WarrantyOption>(
                        devices,
                        'devName',
                        'devSerial'
                      )}
                      value={mapDefaultValue<
                        WarrantyOption,
                        keyof WarrantyOption
                      >(devices, String(devName), 'devName', 'devSerial')}
                      onChange={e =>
                        setWarrantyObject({
                          ...warrantyObject,
                          devName: String(e?.value),
                          devSerial: String(
                            e?.label.substring(0, 3) === 'eTP'
                              ? 'smtrack'
                              : 'i-TeMS'
                          )
                        })
              ...
              <Col sm={12}>
                <Form.Label className='w-100'>
                  {t('customerName')}
                  <Select
                    options={mapOptionsHospital<Hospital, keyof Hospital>(
                      hospitalsData,
                      'id',
                      'hosName'
                    )}
                    value={mapDefaultValueHospital<Hospital, keyof Hospital>(
                      hospitalsData,
                      String(customerName),
                      'id',
                      'hosName'
              ...
              <Col sm={12}>
                <Form.Label className='w-100'>
                  ...
                  <Select
                    options={mapOptionsHospital<Company, keyof Company>(
                      companyList,
                      'key',
                      'value'
                    )}
                    value={mapDefaultValueHospital<Company, keyof Company>(
                      companyList,
                      String(saleDept),
                      'key',
                      'value'
          ...
          <Modal.Footer>
            <FormFlexBtn>
              <FormBtn type='submit'>{t('saveButton')}</FormBtn>
            </FormFlexBtn>
          </Modal.Footer>
        </Form>
      </Modal>
      ...
      <Modal scrollable show={show} onHide={closemodal} size='lg'>
        ...
        <Modal.Body>
          {deviceDetails.length > 0 ? (
            <Printwarranty data={deviceDetails} componentRef={contentRef} />
          ) : (
            <Loading
              loading={false}
              title={t('nodata')}
