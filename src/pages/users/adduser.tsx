...
export default function Adduser (AdduserProp: adduserProp) {
  ...
  const { tokenDecode, cookieDecode } = useSelector(
  ...
  const [form, setform] = useState({
    ...
    user_status:
      pagestate !== 'add' ? (userData?.userStatus === true ? 1 : 0) : 0,
    fileupload: null as File | null
  })
  const [hosid, setHosid] = useState('')
  const [userPicture, setUserPicture] = useState<string>(
    userData?.userPic
      ? `${import.meta.env.VITE_APP_IMG}${userData?.userPic}`
      : ''
  )
  ...
  const reFetchdata = async () => {
    if (tokenDecode.userId) {
      try {
        const response = await axios.get<responseType<usersType>>(
          `${import.meta.env.VITE_APP_API}/user/${tokenDecode.userId}`,
          { headers: { authorization: `Bearer ${token}` } }
        )
        const { displayName, userId, userLevel, userPic, ward, wardId } =
          response.data.data
        ...
        cookies.set(
          'localDataObject',
          String(accessToken(localDataObject)),
          cookieOptions
        )
  ...
  const handleSubmitEdit = async (e: FormEvent) => {
    e.preventDefault()
    const url: string = `${import.meta.env.VITE_APP_API}/user/${
      userData?.userId
    }`
    const formData = new FormData()
    formData.append('wardId', form.group_id)
    ...
    if (
      ...
      try {
        const response = await axios.put<responseType<usersType>>(
          url,
          formData,
          {
            headers: {
    ...
    if (
      ...
      try {
        const response = await axios.patch(
          `${import.meta.env.VITE_APP_API}/auth/reset/${userData?.userId}`,
          {
    ...
    <>
      ...
      <Modal size='lg' show={show} onHide={closeModal}>
        <Modal.Header>
          <ModalHead>
            <strong>
              {pagestate === 'add' ? t('addUserButton') : t('editUserButton')}
            </strong>
                ...
                <InputGroup className='mb-3'>
                  <Form.Label className='w-100'>
                    ...
                    <WardDropdown
                      ...
                      Hosid={
                        pagestate === 'add' ? hosid : String(userData?.hosId)
                      }
              ...
              <Col lg={6}>
                <InputGroup className='mb-3'>
                  <Form.Label className='w-100'>
                    {t('userRole')}
                    <Select
                      options={mapOptions<dataType, keyof dataType>(
                        userlevel,
                        'value',
                        'name'
                      )}
                      value={mapDefaultValue<dataType, keyof dataType>(
                        userlevel,
                        form.user_level,
                        'value',
                        'name'
